...
namespace MyMvcApp.Serverless
{
    public class StripeEventProcessor
    {
        ...
        private async Task<StripeEventProcessResult> HandleCheckoutSessionCompletedAsync(JsonElement stripeEvent, string? eventId, StripeEventSummary summary)
        {
                ...
                if (int.TryParse(bookingIdStr, out var bookingId))
                {
                    ...
                    if (booking != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(booking.GuestEmail))
                        {
                            try
                            {
                                await SendPropertyAccessPassAsync(booking);
                            }
                            catch (Exception ex)
                            {
        ...
        private async Task<StripeEventProcessResult> HandlePaymentIntentSucceededAsync(JsonElement stripeEvent, string? eventId, StripeEventSummary summary)
        {
            var dataObject = GetDataObject(stripeEvent);
            ...
            AddAuditLog("StripePaymentIntentSucceeded", payment.PaymentId, $"Processed payment_intent.succeeded from Amazon EventBridge. PaymentIntent={paymentIntentId}.");

            await _context.SaveChangesAsync();
            return StripeEventProcessResult.Ok(new { processed = true, payment.PaymentId, payment.Status }, summary);
        }

        private async Task<StripeEventProcessResult> HandlePaymentIntentFailedAsync(JsonElement stripeEvent, string? eventId, StripeEventSummary summary)
        {
            var dataObject = GetDataObject(stripeEvent);
            var paymentIntentId = ReadString(dataObject, "id");
            var payment = await FindPaymentAsync(dataObject, null, paymentIntentId);
        ...
        private async Task<StripeEventProcessResult> HandleCheckoutSessionRejectedAsync(JsonElement stripeEvent, string? eventId, PaymentStatus status, string message, StripeEventSummary summary)
        {
            var dataObject = GetDataObject(stripeEvent);
            var sessionId = ReadString(dataObject, "id");
            var paymentIntentId = ReadExpandableString(dataObject, "payment_intent");
        ...
        private async Task<StripeEventProcessResult> HandleChargeRefundedAsync(JsonElement stripeEvent, string? eventId, StripeEventSummary summary)
        {
            var dataObject = GetDataObject(stripeEvent);
        ...
        private async Task<StripeEventProcessResult> HandleRefundCreatedAsync(JsonElement stripeEvent, string? eventId, StripeEventSummary summary)
        {
            var dataObject = GetDataObject(stripeEvent);
            var paymentIntentId = ReadExpandableString(dataObject, "payment_intent") ?? ReadString(dataObject, "payment_intent");
            var payment = await FindPaymentAsync(dataObject, null, paymentIntentId);
        ...
        private async Task SendPropertyAccessPassAsync(PropertyBooking booking)
        {
            var passCode = booking.PassCode;
            if (string.IsNullOrWhiteSpace(passCode))
            {
                return;
            }

            var senderEmail = _configuration["AWS:SesSenderEmail"];
            if (string.IsNullOrWhiteSpace(senderEmail))
            {
                _logger.LogWarning("SES sender email is not configured.");
                return;
            }

            var region = RegionEndpoint.GetBySystemName(_configuration["AWS:Region"] ?? "ap-southeast-1");
            var qrImageUrl = await UploadQrPassAsync(booking, passCode, region);
            var subject = $"Stay Confirmed: {booking.Property.PropertyName}";
            var htmlBody = $@"
                <h2>Stay Confirmed!</h2>
                <p>Hello {booking.GuestName},</p>
                <p>Your payment was successful. Please present this pass code at check in.</p>
                <p><strong>Property:</strong> {booking.Property.PropertyName}</p>
                <p><strong>Check-In:</strong> {booking.CheckInDate:dd MMM yyyy} at 3:00 PM</p>
                <p><strong>Check-Out:</strong> {booking.CheckOutDate:dd MMM yyyy} at 11:00 AM</p>
                <p><strong>Pass Code:</strong> {passCode}</p>
                <p><a href='{qrImageUrl}'>View Digital Pass</a></p>
                <p>Address: {booking.Property.AddressLine1}, {booking.Property.City}</p>";
            ...
            {
                Source = senderEmail,
                Destination = new Destination { ToAddresses = new List<string> { booking.GuestEmail } },
                Message = new Message
                {
        ...
        private async Task<string> UploadQrPassAsync(PropertyBooking booking, string passCode, RegionEndpoint region)
        {
